...
import { getJapanHolidayKeysForYear } from "@/lib/japan-holidays";
...
/**
 * 祝日の計算は japan-holidays.ts へ移した（タスクV）。
 * キャンセル処理の確認画面がクライアント側でも営業日を数えるため、
 * server-only でないモジュールから使えるようにする必要があった。
 * 中身は変えていない。
 */
