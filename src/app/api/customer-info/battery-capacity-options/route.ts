...
import {
  fetchBatteryCapacityOptionsForManufacturer,
...
export async function GET(request: Request) {
  ...
  try {
    const options =
      await fetchBatteryCapacityOptionsForManufacturer(manufacturer);
    if (options === null) {
      return NextResponse.json({
        options: [],
        configured: false,
    ...
    console.error("[api/customer-info/battery-capacity-options]", e);
    return NextResponse.json(
      { error: "蓄電池容量一覧の取得に失敗しました" },
      { status: 502 },
    );
  }
}
