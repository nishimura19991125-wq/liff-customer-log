 ...
 * 新規案件通知（Google Chat）。
 *
 * 送る条件は「T番号 が新規発行されたか」の一本。採番するのはお客様情報
 * アプリで、採番されるのはお客様情報レコードを新規作成したときだけ。
 * このルートは必ず新規発行になるので、施工予定日の有無で経路が分かれても
 * **どちらでも送る**。
 *
 * 空き枠入力（fill-empty-slot）は連携が新規作成したときだけ送る
 * （判定は finalizeConstructionCalendarSave 側。ここでは扱わない）。
 * 未定案件の割り当て・工事日の移動は既存の T番号 を使い回すので、
